...
namespace Async {
    ...
    class TaskFactory {
        public:
            ...
            template <typename T>
            auto create(std::function<T(void)> function) -> Task<T>;
...
template <typename T>
auto inline Async::TaskFactory::value_source() -> TaskValueSource<T> { return TaskValueSource<T>(*scheduler); }
auto inline Async::TaskFactory::timer_source() -> TaskTimerSource { return { *scheduler, *timing_poll_source }; }
auto inline Async::TaskFactory::io_source() -> TaskIOSource { return { *scheduler, *io_poll_source }; }


template <typename T>
auto Async::TaskFactory::create(std::function<T(void)> function) -> Task<T> { return Task<T>(*scheduler, function); }
