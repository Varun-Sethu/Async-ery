    ...
    cells(std::move(cells))
{
    ...
    for (size_t cell_idx = 0; cell_idx < this->cells.size(); cell_idx += 1) {
        this->cells[cell_idx]->await([execution_context, cell_idx, underlying_cell=this->underlying_cell](auto ctx, T value) {
            ...
            auto& resolved_values = execution_context->resolved_values();
            auto& num_resolved_tasks = execution_context->num_resolved_cells();
            auto total_cells = execution_context->total_cells();
